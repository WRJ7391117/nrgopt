...
test('cross-check output must reference facts that exist on both saved sources', () => {
  const value = { same_project: true, matching_facts: [{ left_fact_number: 1, right_fact_number: 1, reason_zh: '项目名称与容量一致。' }], conflicting_facts: [] };
  ...
  assert.throws(() => validateCrossCheck({ same_project: true, matching_facts: [], conflicting_facts: [] }, 1, 1), { code: 'cross_check_invalid' });
});

test('DeepSeek cross-check sends only bounded saved evidence and validates the response', async () => {
  let request;
  const expected = { same_project: true, matching_facts: [{ left_fact_number: 1, right_fact_number: 1, reason_zh: '两边均披露每个项目500 MW/2,000 MWh。' }], conflicting_facts: [] };
