...
test('operations exposes recent owner-scoped jobs, item checkpoints and budget state', async () => {
  ...
  const fetchImpl = async (input) => {
    ...
    if (url.pathname.endsWith('/intelligence_budget_accounts')) return new Response(JSON.stringify([
      { currency: 'CNY', limit_micro: 5000, reserved_micro: 0, spent_micro: 1000, enabled: true },
      { currency: 'USD', limit_micro: 2000, reserved_micro: 0, spent_micro: 0, enabled: false }
    ]));
