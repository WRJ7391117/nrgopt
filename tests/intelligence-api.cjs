...
function setup({ overrides = {}, environment = env, sourceFetcher, modelFactory, crossCheckFactory, discoveryFactory,
  ...
  const store = {
    ...
    candidates: async () => [], operations: async () => ({ runs: [], items: [], budgets: [], notifications: [] }), candidateBySource: async () => null, previousExtractedSource: async () => null, findCandidatePeers: async () => [], saveCrossCheck: async () => ({}),
