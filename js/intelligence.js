  ...
  function renderOperations(result) {
    ...
    result.runs.forEach(function (run) {
      ...
      detail.textContent = children.map(function (child) {
        ...
        var errors = { discovery_balance_insufficient: '服务商返回余额不足，请核对密钥和套餐权限', discovery_failed: '来源发现暂未成功', source_failed: '原文获取失败', extraction_failed: '提取或证据校验失败', cross_check_failed: '跨来源核对失败', lease_exhausted: '多次执行超时，已停止自动重试', budget_exhausted: '预算不足', billing_sync_pending: '等待账单同步' };
  ...
  if (extractButton) {
    extractButton.addEventListener('click', async function () {
      ...
      try {
        ...
        renderExtraction(result.source);
