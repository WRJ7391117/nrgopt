...
function createFeishuSender({ webhookUrl, appId, appSecret, chatId, fetchImpl = global.fetch }) {
  let endpoint = null;
  if (webhookUrl) {
  ...
  if (!/^[A-Za-z0-9_-]{6,200}$/.test(appId || '') || typeof appSecret !== 'string' || appSecret.length < 8 || appSecret.length > 500 ||
      (chatId && !/^oc_[A-Za-z0-9_-]{8,200}$/.test(chatId))) throw failure('feishu_not_configured');
  return async input => {
    let tokenResponse;
    try {
    ...
    } catch { throw failure('delivery_failed'); }
    if (!tokenResponse.ok) throw failure('delivery_failed', tokenResponse.status);
    let tokenPayload;
    try { tokenPayload = await tokenResponse.json(); } catch { throw failure('delivery_failed', tokenResponse.status); }
    if (tokenPayload?.code !== 0 || typeof tokenPayload.tenant_access_token !== 'string') throw failure('delivery_failed', tokenResponse.status);
    ...
    if (!target) {
      ...
      if (!chatsResponse.ok) throw failure('delivery_failed', chatsResponse.status);
      let chatsPayload;
      try { chatsPayload = await chatsResponse.json(); } catch { throw failure('delivery_failed', chatsResponse.status); }
      const chats = chatsPayload?.code === 0 && Array.isArray(chatsPayload?.data?.items) ? chatsPayload.data.items : [];
      if (chatsPayload?.data?.has_more || chats.length !== 1 || !/^oc_[A-Za-z0-9_-]{8,200}$/.test(chats[0]?.chat_id || '')) {
    ...
    let response;
    try {
      response = await fetchImpl('https://open.feishu.cn/open-apis/im/v1/messages?receive_id_type=chat_id', {
        method: 'POST', redirect: 'error', signal: AbortSignal.timeout(10000),
        headers: { Authorization: authorization, 'Content-Type': 'application/json' },
        body: JSON.stringify({ receive_id: target, msg_type: 'interactive', content: JSON.stringify(buildCard(input).card) })
      });
    } catch { throw failure('delivery_unknown'); }
    if (!response.ok) throw failure('delivery_failed', response.status);
    let payload;
    try { payload = await response.json(); } catch { throw failure('delivery_unknown', response.status); }
    if (payload?.code !== 0) throw failure('delivery_failed', response.status);
    return { responseCode: response.status, messageId: payload?.data?.message_id || null, chatId: target };
  };
}
