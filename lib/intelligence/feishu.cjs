  ...
  return async input => {
    ...
    if (!target) {
      ...
      if (chatsPayload?.data?.has_more || chats.length !== 1 || !/^oc_[A-Za-z0-9_-]{8,200}$/.test(chats[0]?.chat_id || '')) {
        throw failure('feishu_chat_target_required');
      }
    ...
    if (!response.ok) throw failure('feishu_send_rejected', response.status);
    let payload;
    try { payload = await response.json(); } catch { throw failure('delivery_unknown', response.status); }
    if (payload?.code !== 0) throw failure('feishu_send_rejected', response.status);
