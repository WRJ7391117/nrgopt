...
async function extractSavedSource({ store, owner, sourceId, env, modelFactory, crossCheckFactory, jobId = null,
  idempotencyPrefix = `manual:${sourceId}:${randomUUID()}` }) {
  ...
  let result;
  try {
    result = await runPaidCall({ store, owner, jobId, operation: 'extraction', currency: 'CNY',
      budgetKey: 'NRGOPT_DEEPSEEK_EXTRACTION_RESERVE_MICROCNY', providerMissingCode: 'model_not_configured', env,
      idempotencyKey: `${idempotencyPrefix}:extraction`, call: async () => {
        await store.beginExtraction(source.id, owner);
        started = true;
        return modelFactory({ apiKey: env.DEEPSEEK_API_KEY })({ title: source.title, url: source.final_url, sourceText });
      } });
  } catch (error) {
  ...
  if (peers.length) {
    const crossCheck = crossCheckFactory({ apiKey: env.DEEPSEEK_API_KEY });
    for (const peer of peers) {
      try {
        const checked = await runPaidCall({ store, owner, jobId, operation: 'cross_check', currency: 'CNY',
          budgetKey: 'NRGOPT_DEEPSEEK_CROSS_CHECK_RESERVE_MICROCNY', providerMissingCode: 'model_not_configured', env,
