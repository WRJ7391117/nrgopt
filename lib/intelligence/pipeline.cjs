...
async function extractSavedSource({ store, owner, sourceId, env, modelFactory, crossCheckFactory, jobId = null,
  ...
  try {
    ...
    const code = ['model_not_configured', 'model_auth_failed', 'model_unavailable', 'model_rate_limited', 'model_timeout', 'extraction_invalid'].includes(error?.code)
...
async function crossCheckSavedSources({ store, owner, leftSourceId, rightSourceId, env, jobId,
  ...
  if (!distinctSourcePublishers(leftSource, rightSource)
    && !plausibleCorrectionPair({ ...left, extraction_zh: leftSource.extraction_zh },
      { ...right, extraction_zh: rightSource.extraction_zh })) return;
