...
async function extractSavedSource({ store, owner, sourceId, env, modelFactory, crossCheckFactory, jobId = null,
  ...
  let result;
  try {
    result = await runPaidCall({ store, owner, jobId, operation: 'extraction', currency: providers.analysis.currency,
      budgetKey: providers.analysis.reserveKey, providerMissingCode: 'model_not_configured', env,
      idempotencyKey: `${idempotencyPrefix}:extraction`, call: async () => {
        await store.beginExtraction(source.id, owner);
        started = true;
        return modelFactory(providers.analysis)({ title: source.title, url: source.final_url, sourceText });
      } });
  } catch (error) {
  ...
  if (peers.length) {
    const crossCheck = crossCheckFactory(providers.analysis);
    for (const peer of peers) {
      try {
        const checked = await runPaidCall({ store, owner, jobId, operation: 'cross_check', currency: providers.analysis.currency,
          budgetKey: providers.analysis.crossCheckReserveKey, providerMissingCode: 'model_not_configured', env,
