...
<head>
  ...
  <script src="/js/intelligence.js?v=20260925-early-opportunity-status" defer></script>
</head>
...
function overviewPage() {
  ...
  return page('六国情报全景', `
    ...
    <section id="opportunities-section" class="intel-panel">
      <h2>早期、设备与服务机会</h2>
      <p class="intel-muted">早期机会由原文事实触发并关联待验证假设，不表示采购已开放。设备与服务机会要求具体包件原文。这里只列最近 100 条分析窗口；按来源逐条列出，同名跨来源尚未合并，不代表独立机会总数；采购开放仍需核对参与资格。</p>
      <ul id="overview-opportunities" class="intel-analysis-list"></ul>
      <p id="overview-opportunities-empty" class="intel-muted" hidden>当前窗口没有带原文依据的机会。早期需求仍在三雷达中跟踪，不自动编造采购机会。</p>
    </section>
  `, true);
}
