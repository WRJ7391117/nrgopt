...
<head>
  ...
  <script src="/js/intelligence.js?v=20260925-opportunities" defer></script>
</head>
<body class="intelligence-app">
  ...
  <header class="intel-header">
    ...
    ${authenticated ? '<nav class="intel-nav" aria-label="情报模块"><a href="/intelligence/overview">六国全景</a><a href="/intelligence/overview#opportunities-section">机会</a><a href="/intelligence">来源工作台</a><a href="/intelligence/settings">服务配置</a></nav>' : ''}
...
function overviewPage() {
  ...
  return page('六国情报全景', `
    ...
    <section class="intel-panel" aria-labelledby="operations-title">
      ...
      <p class="intel-muted">每日安排六国基础搜索，并为最多 5 组关注对象分别搜索支持证据和反证，合计最多 16 组查询（失败重试另计）。关注对象按日轮换，每 90 天自动复查，无有效新证据则休眠，有有效新证据则延长观察；未搜到新消息不代表假设被否定。</p>
      ...
      <p class="intel-muted">固定来源监测与搜索同时进行：每个入口分别补收链接，并复查近期公告的更正。下方成功表示链接已登记，正文和事实仍需继续核验；读取失败不等于没有新消息。</p>
    ...
    <form id="candidate-filters" class="intel-panel">
      <div class="intel-settings-grid">
        ...
        <label>雷达<select name="radar"><option value="">全部雷达</option><option value="trigger">触发雷达</option><option value="demand">需求雷达</option><option value="project">项目雷达</option></select></label>
      </div>
      <p id="candidate-filter-status" class="intel-status" role="status" aria-live="polite">正在读取候选…</p>
      <button type="reset" class="intel-button intel-button-quiet">返回六国全景</button>
      <p class="intel-muted">筛选只改变下方列表，上方六国与雷达统计保留全景。同一条情报可以属于多个国家或雷达。</p>
    </form>
    <p class="intel-muted">AI 负责发现、提取和基础证据核验。点击标题即可查看来源链接、原文摘录和逐条引文；“跨来源内容已比对”只表示列出的表述已做比较；不同网站可能转载同一消息，来源独立性仍未确认。你不需要逐条审批。</p>
    ...
    <section id="opportunities-section" class="intel-panel">
      <h2>设备与服务机会</h2>
      <p class="intel-muted">只列最近 100 条分析窗口中有原文依据的具体设备或服务包。这里按来源逐条列出，同名跨来源尚未合并，不代表独立机会总数；采购开放仍需核对参与资格。</p>
      <ul id="overview-opportunities" class="intel-analysis-list"></ul>
      <p id="overview-opportunities-empty" class="intel-muted" hidden>当前窗口没有已核实包件。早期需求仍在三雷达中跟踪，不自动编造采购机会。</p>
    </section>
  `, true);
}
...
function settingsPage() {
  return page('模型服务配置', `
    ...
    <h1>模型服务配置</h1>
    <p class="intel-intro">在这里决定系统使用哪家 API、实际购买方式和月度金额上限。中国模型按人民币记账，美国模型按美元记账。</p>
...
function sourcesPage({ detailId = null } = {}) {
  const content = detailId ? `
    ...
    <article id="source-detail" hidden>
      ...
      <section id="opportunities-section" class="intel-panel" hidden><h2>可参与机会</h2><p class="intel-muted">只列出本来源有原文证据的设备或服务包。采购开放仅表示程序已披露，仍需核对参与资格；EPC 授标不会自动关闭未披露的设备机会。未再次确认不等于取消。状态记录时间是系统分析时间，不是市场公告日期。</p><ul id="opportunities-list" class="intel-analysis-list"></ul></section>
