...
function settingsPage() {
  return page('模型服务配置', `
    ...
    <h1>模型服务配置</h1>
    <p class="intel-intro">在这里决定系统使用哪家 API，并为每项服务设置月度金额上限。中国模型按人民币记账，美国模型按美元记账。</p>
    <div class="intel-notice"><h2>金额上限怎么生效</h2><p>“来源发现”和“情报分析”分别计算当月用量。达到各自上限后，系统会在发起下一次付费调用前停止；每次调用需要预留多少由后台自动处理。</p></div>
