...
<head>
  ...
  <script src="/js/intelligence.js?v=20260924-registry" defer></script>
</head>
...
function providerForm(capability, title, description, endpointHelp) {
  return `<form class="intel-panel intel-provider-form" data-capability="${capability}" method="post" action="/api/intelligence?action=save-provider-settings">
    ...
    <div class="intel-settings-grid">
      ...
      <label>计费方式<select name="billing_mode" required><option value="balance">按量扣费（读取供应商余额）</option><option value="included">已购套餐（单次金额为 0）</option></select><span class="intel-field-help">由你按实际购买方式选择；系统不会再根据 token 自行估算金额。目前按量账单自动同步已适配 DeepSeek 官方 API。</span></label>
...
function settingsPage() {
  return page('模型服务配置', `
    ...
    <div class="intel-notice"><h2>金额以哪里为准</h2><p>按量扣费只记录服务商余额接口确认的实扣金额；已购套餐的单次金额记为 0。模型返回的 token 仅用于调用记录，不换算成消费金额。</p></div>
