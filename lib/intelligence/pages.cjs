...
<body class="intelligence-app">
  ...
  <header class="intel-header">
    ...
    ${authenticated ? '<nav class="intel-nav" aria-label="情报模块"><a href="/intelligence/overview">六国全景</a><a href="/intelligence">来源工作台</a></nav>' : ''}
...
function sourcesPage({ detailId = null } = {}) {
  const content = detailId ? `
    ...
    <article id="source-detail" hidden>
      ...
      <section class="intel-panel intel-analysis">
        <div class="intel-section-heading">
          <div><p class="intel-kicker">DeepSeek · 单一来源分析</p><h2>中文情报与证据</h2></div>
    ...
    <section class="intel-guide" aria-labelledby="guide-title">
      ...
      <ol>
        <li><strong>AI 发现来源</strong><span>MiniMax 联网搜索政府、采购方、业主和项目公司的原始发布。</span></li>
        <li><strong>保存原文证据</strong><span>系统抓取正文、计算指纹并保存来源链接和原件。</span></li>
        <li><strong>生成中文情报</strong><span>DeepSeek 提取事实、三雷达分类和逐条原文引文。</span></li>
      </ol>
    ...
    <form id="import-form" class="intel-panel" method="post" action="/api/intelligence?action=import">
      ...
      <p id="import-help" class="intel-muted">填写可公开访问的 HTTPS 链接，支持 UTF-8 网页或纯文本，暂不支持 PDF。只有点击“生成中文初析”时，已保存的公开来源文本才会发送给 DeepSeek。</p>
...
module.exports = { loginPage, sourcesPage, overviewPage };
