...
function createMiniMaxDiscoverer({ apiKey, fetchImpl = global.fetch } = {}) {
  return async function discover({ query }) {
    if (!apiKey) throw failure('minimax_not_configured', 503);
    let response;
    try {
    ...
    if ([401, 403].includes(response.status)) throw failure('minimax_auth_failed', 502);
    ...
    return {
      results: results.slice(0, 12), model: payload.model || 'MiniMax-M3', search_count: searchCount,
      usage: {
...
module.exports = { createMiniMaxDiscoverer, validResult };
