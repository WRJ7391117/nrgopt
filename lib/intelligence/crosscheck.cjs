...
function plausibleCorrectionPair(left, right) {
  ...
  const correction = /更正|修正|更新|延期|延长|取消|撤销|重新(?:邀请|发布|开放)|\b(?:corrected|revised|amended|extended|postponed|cancelled|canceled|reissued|reinvited)\b/i;
  if (!correction.test(leftText) && !correction.test(rightText)) return false;
...
module.exports = { createDeepSeekCrossChecker, plausibleSameProject, plausibleCorrectionPair, correctionContext,
