...
function createDeepSeekCrossChecker({ apiKey, fetchImpl = global.fetch } = {}) {
  ...
  return async function crossCheck({ left, right }) {
    ...
    try {
      response = await fetchImpl(ENDPOINT, {
        ...
        body: JSON.stringify({
          model: MODEL, thinking: { type: 'disabled' }, temperature: 0, max_tokens: 1800, response_format: { type: 'json_object' },
          messages: [
    ...
    return { ...validateCrossCheck(parsed, leftFacts.length, rightFacts.length), provider: 'deepseek', model: payload.model || MODEL,
...
module.exports = { createDeepSeekCrossChecker, plausibleSameProject, validateCrossCheck };
