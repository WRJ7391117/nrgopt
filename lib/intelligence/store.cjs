...
function createStore(config, fetchImpl = global.fetch) {
  ...
  async function attachRelations(candidates, owner) {
    ...
    const sources = await rows(`owner_id=eq.${encodeURIComponent(owner)}&id=in.(${sourceIds.map(encodeURIComponent).join(',')})&select=id,final_url,content_sha256,extracted_at`);
  ...
  return {
    ...
    async analysisRevisions(sourceId, owner) {
      ...
      return tableRows('intelligence_analysis_revisions', `owner_id=eq.${encodeURIComponent(owner)}&source_id=eq.${encodeURIComponent(sourceId)}&select=id,revision_no,source_sha256,extraction_zh,provider,model,extracted_at,recorded_at&order=revision_no.desc&limit=50`);
    },
