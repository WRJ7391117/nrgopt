...
function createStore(config, fetchImpl = global.fetch) {
  ...
  return {
    ...
    async analysisRevisions(sourceId, owner) {
      ...
      return tableRows('intelligence_analysis_revisions', `owner_id=eq.${encodeURIComponent(owner)}&source_id=eq.${encodeURIComponent(sourceId)}&select=id,source_sha256,extraction_zh,provider,model,extracted_at,recorded_at&order=recorded_at.desc,id.desc&limit=50`);
    },
