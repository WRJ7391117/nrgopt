...
function createStore(config, fetchImpl = global.fetch) {
  ...
  async function attachRelations(candidates, owner) {
    ...
    const sources = await rows(`owner_id=eq.${encodeURIComponent(owner)}&id=in.(${sourceIds.map(encodeURIComponent).join(',')})&select=id,final_url,content_sha256,extracted_at,fetched_at,extraction_zh`);
    ...
    return candidates.map(item => {
      ...
      const evidenceStatus = retained.some(link => link.relation === 'conflicts') ? 'conflict'
        : retained.length ? 'checked' : singleSourceStatus(item);
      ...
      independence: 'unverified', shared_quote_count: sharedQuotes(link),
    ...
    async candidates(owner) {
      ...
      for (const item of linked) {
        ...
        for (const field of ['radars', 'occurrence_countries', 'relevance_countries']) {
          item[field] = [...new Set(members.flatMap(member => member[field] || []))];
        }
        item.grouped_sources = members.map(member => ({ candidate_id: member.id, source_id: member.source_id }));
        grouped.push(item);
      }
      return grouped;
    },
