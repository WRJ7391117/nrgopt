...
function createStore(config, fetchImpl = global.fetch) {
  ...
  async function attachRelations(candidates, owner) {
    ...
    const sources = await rows(`owner_id=eq.${encodeURIComponent(owner)}&id=in.(${sourceIds.map(encodeURIComponent).join(',')})&select=id,final_url,content_sha256`);
    ...
    return candidates.map(item => {
      ...
      const retained = valid.filter(link => link.candidate_id === item.id);
  ...
  return {
    ...
    async candidateBySource(sourceId, owner) {
      ...
      const [hypotheses, watches] = await Promise.all([
        tableRows('intelligence_hypotheses', `${scope}&select=id,claim_zh,counter_evidence_zh,status,created_at&order=created_at.asc`),
    ...
    async watchSearchTargets(owner, now = new Date()) {
      const since = new Date(new Date(now).getTime() - 90 * 86400000).toISOString();
      const hypotheses = await tableRows('intelligence_hypotheses', `owner_id=eq.${encodeURIComponent(owner)}&status=in.(open,strengthened,weakened)&created_at=gt.${encodeURIComponent(since)}&select=id,candidate_id,claim_zh,counter_evidence_zh,status,created_at&order=created_at.desc,id.asc&limit=100`);
    ...
    async findCandidatePeers(candidate, owner) {
      ...
      for (const item of peers) {
        ...
        independent.push({ ...item, extraction_zh: other.extraction_zh });
