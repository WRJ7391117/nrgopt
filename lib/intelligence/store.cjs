  ...
  return {
    ...
    async operations(owner) {
      ...
      const budgetRows = await tableRows('intelligence_provider_configs',
        `owner_id=eq.${encodeURIComponent(owner)}&select=capability,currency,billing_mode,budget_limit_micro,budget_reserved_micro,budget_spent_micro,budget_period_start,budget_period_end,budget_enabled,provider_balance_synced_at&order=capability.asc`);
