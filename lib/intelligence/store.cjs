...
const FIELDS = 'id,title,requested_url,final_url,fetched_at,published_at,status,error_code,excerpt,content_type,byte_size,content_sha256,archive_status,archived_at,annotation_zh,annotation_updated_at,extraction_status,extraction_zh,extraction_provider,extraction_model,extraction_source_sha256,extraction_error_code,extracted_at';
...
const RELATION_FIELDS = 'candidate_id,related_candidate_id,relation,matching_facts_zh,conflicting_facts_zh,checked_at';
const PROVIDER_CONFIG_FIELDS = 'owner_id,capability,provider,endpoint,model,currency,budget_limit_micro,budget_reserved_micro,budget_spent_micro,budget_period_start,budget_period_end,budget_enabled,api_key_ciphertext,updated_at';
...
function createStore(config, fetchImpl = global.fetch) {
  ...
  async function attachRelations(candidates, owner) {
    ...
    return candidates.map(item => ({ ...item, related_sources: linked.filter(link => link.candidate_id === item.id).map(link => ({
  ...
  return {
    ...
    async saveExtraction(id, owner, result, sourceHash) {
      await update(id, owner, { extraction_status: 'extracted', extraction_zh: result.extraction,
        extraction_provider: result.provider, extraction_model: result.model, extraction_source_sha256: sourceHash,
        extraction_usage: result.usage, extraction_error_code: null, extracted_at: new Date().toISOString() });
      return one(id, owner);
    },
    async saveCandidate(sourceId, owner, extraction, sourceHash) {
      ...
      const childFilter = `owner_id=eq.${encodeURIComponent(owner)}&candidate_id=eq.${encodeURIComponent(candidate.id)}`;
      await tableRows('intelligence_hypotheses', childFilter, { method: 'DELETE' });
      await tableRows('intelligence_watch_targets', childFilter, { method: 'DELETE' });
      if (extraction.hypotheses.length) await tableRows('intelligence_hypotheses', '', { method: 'POST', body: JSON.stringify(extraction.hypotheses.map(item => ({
        owner_id: owner, candidate_id: candidate.id, claim_zh: item.hypothesis_zh, counter_evidence_zh: item.counter_evidence_zh || null
      }))), headers: { Prefer: 'return=minimal' } });
      if (extraction.next_signals_zh.length) await tableRows('intelligence_watch_targets', '', { method: 'POST', body: JSON.stringify(extraction.next_signals_zh.map(item => ({
        owner_id: owner, candidate_id: candidate.id, signal_zh: item
      }))), headers: { Prefer: 'return=minimal' } });
    ...
    async candidates(owner) {
      ...
      return attachRelations(candidates, owner);
    },
    async operations(owner) {
      ...
      const budgetRows = await tableRows('intelligence_provider_configs',
        `owner_id=eq.${encodeURIComponent(owner)}&select=capability,currency,budget_limit_micro,budget_reserved_micro,budget_spent_micro,budget_period_start,budget_period_end,budget_enabled&order=capability.asc`);
      const budgets = budgetRows.map(item => ({ capability: item.capability, currency: item.currency,
    ...
    async candidateBySource(sourceId, owner) {
      const candidates = await tableRows('intelligence_candidates', `owner_id=eq.${encodeURIComponent(owner)}&select=${CANDIDATE_FIELDS}&order=updated_at.desc&limit=100`);
      return (await attachRelations(candidates, owner)).find(item => item.source_id === sourceId) || null;
    },
    ...
    async claimJobItem(owner, jobId, leaseSeconds = 240) {
      const result = await request('/rest/v1/rpc/claim_intelligence_job_item', { method: 'POST', body: JSON.stringify({
        p_owner_id: owner, p_job_run_id: jobId, p_lease_seconds: leaseSeconds
      }) });
      return result?.[0] || null;
    },
    async finishJobItem(owner, itemId, status, checkpoint = {}, errorCode = null) {
      return request('/rest/v1/rpc/finish_intelligence_job_item', { method: 'POST', body: JSON.stringify({
        p_owner_id: owner, p_item_id: itemId, p_status: status, p_checkpoint: checkpoint, p_error_code: errorCode
      }) });
    },
    ...
    async findCandidatePeers(candidate, owner) {
      ...
      const peers = result.filter(item => plausibleSameProject(candidate, item)).slice(0, 3);
      return Promise.all(peers.map(async item => ({ ...item, extraction_zh: (await one(item.source_id, owner, true)).extraction_zh })));
    },
    async saveCrossCheck(candidateId, relatedCandidateId, owner, checked) {
      ...
      const records = [
        { owner_id: owner, candidate_id: candidateId, related_candidate_id: relatedCandidateId, relation,
          matching_facts_zh: checked.matching_facts, conflicting_facts_zh: checked.conflicting_facts, checked_at: now },
        { owner_id: owner, candidate_id: relatedCandidateId, related_candidate_id: candidateId, relation,
          matching_facts_zh: reverse(checked.matching_facts), conflicting_facts_zh: reverse(checked.conflicting_facts), checked_at: now }
      ];
    ...
    async save(source, owner) {
      const record = { id: randomUUID(), owner_id: owner, requested_url: source.requestedUrl, final_url: source.finalUrl,
        title: source.title, fetched_at: new Date().toISOString(), published_at: null, status: 'saving_evidence',
