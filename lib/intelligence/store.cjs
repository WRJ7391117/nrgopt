...
function settings(env = process.env) {
  ...
  const previewAllowed = env.VERCEL_ENV !== 'preview' || env.NRGOPT_INTELLIGENCE_PREVIEW_WRITE_ENABLED === '1';
  ...
  return { url: url.origin, origin: origin.origin, anonKey: env.SUPABASE_ANON_KEY, serviceKey: env.SUPABASE_SERVICE_ROLE_KEY,
    adminId: env.NRGOPT_ADMIN_USER_ID, origins: [...origins], writes: writesRequested && productionAllowed && previewAllowed };
}
...
function createStore(config, fetchImpl = global.fetch) {
  async function request(path, options = {}) {
    ...
    if (!response.ok) {
      ...
      if (path.startsWith('/rest/v1/') && response.status === 400) {
        let code;
        try { code = (await response.json()).code; } catch { /* Do not expose raw upstream errors. */ }
        if (code === '23514') throw failure('storage_constraint');
      }
  ...
  async function attachRelations(candidates, owner) {
    ...
    const singleSourceStatus = item => ['checked', 'conflict', 'corrected'].includes(item.evidence_status) ? 'sourced' : item.evidence_status;
    if (!linked.length) return candidates.map(item => ({ ...item, evidence_status: singleSourceStatus(item), related_sources: [] }));
    const missing = [...new Set(linked.map(item => item.related_candidate_id).filter(id => !byId.has(id)))];
    if (missing.length) {
      const peers = await tableRows('intelligence_candidates', `owner_id=eq.${encodeURIComponent(owner)}&id=in.(${missing.map(encodeURIComponent).join(',')})&select=${CANDIDATE_FIELDS}`);
      for (const peer of peers) byId.set(peer.id, peer);
    }
    const { distinctSourcePublishers, plausibleCorrectionPair } = require('./crosscheck.cjs');
    const sourceIds = [...new Set([...byId.values()].map(item => item.source_id))];
    const sources = await rows(`owner_id=eq.${encodeURIComponent(owner)}&id=in.(${sourceIds.map(encodeURIComponent).join(',')})&select=id,final_url,content_sha256,extracted_at,fetched_at,publication_date,extraction_zh`);
    const sourceFor = id => sources.find(source => source.id === byId.get(id)?.source_id);
    const correctionLink = link => {
      ...
      return left && right && !distinctSourcePublishers(left, right)
        && plausibleCorrectionPair({ ...byId.get(link.candidate_id), extraction_zh: left.extraction_zh },
          { ...byId.get(link.related_candidate_id), extraction_zh: right.extraction_zh });
    };
    const sharedQuotes = link => {
      const left = sourceFor(link.candidate_id)?.extraction_zh?.known_facts || [];
      const right = sourceFor(link.related_candidate_id)?.extraction_zh?.known_facts || [];
      const normalized = quote => typeof quote === 'string' ? quote.replace(/\s+/gu, ' ').trim() : '';
      return (link.matching_facts_zh || []).filter(pair => {
        const quote = normalized(left[pair.left_fact_number - 1]?.evidence_quote);
        return quote && quote === normalized(right[pair.right_fact_number - 1]?.evidence_quote);
      }).length;
    };
    const valid = linked.filter(link => {
      const left = sourceFor(link.candidate_id), right = sourceFor(link.related_candidate_id);
      return left && right && (distinctSourcePublishers(left, right) || correctionLink(link))
    ...
    return candidates.map(item => {
      ...
      const evidenceStatus = retained.some(link => link.relation === 'conflicts') ? 'conflict'
        : retained.some(correctionLink) ? 'corrected' : retained.length ? 'checked' : singleSourceStatus(item);
      ...
      independence: correctionLink(link) ? 'same_publisher' : 'unverified', shared_quote_count: sharedQuotes(link),
  ...
  async function materializeBusinessObjects(candidate, classification, owner, extraction, sourceHash) {
    ...
    if (!project?.name_zh || !projectCountry) {
      const previousProjects = await tableRows('intelligence_projects', projectFilter + '&select=id');
      await tableRows('intelligence_projects', projectFilter + '&current_in_analysis=eq.true', { method: 'PATCH', body: JSON.stringify({ current_in_analysis: false }) });
      const early = classification.disposition === 'candidate' && !project && !classification.procurement
        ? (classification.early_opportunities || []) : [];
      const hypotheses = early.length ? await tableRows('intelligence_hypotheses',
        projectFilter + '&select=id,claim_zh,counter_evidence_zh') : [];
      const currentEarly = early.map(item => {
        const stated = extraction.hypotheses[item.hypothesis_number - 1];
        const hypothesis = hypotheses.find(row => row.claim_zh === stated?.hypothesis_zh
          && (row.counter_evidence_zh || '') === (stated?.counter_evidence_zh || ''));
        const quote = extraction.known_facts?.[item.evidence_fact_number - 1]?.evidence_quote;
        if (!hypothesis?.id || !quote) throw failure('storage_constraint');
        return { ...item, hypothesis_id: hypothesis.id, evidence_quote: quote };
      });
      const previousOpportunities = await tableRows('intelligence_opportunities', projectFilter + '&current_in_analysis=eq.true&select=id,scope,package_name_zh');
      for (const previous of previousOpportunities) if (!currentEarly.some(item =>
        previous.scope === 'early' && previous.package_name_zh === item.opportunity_zh)) {
        await tableRows('intelligence_opportunities', projectFilter + '&id=eq.' + encodeURIComponent(previous.id), {
          method: 'PATCH', body: JSON.stringify({ current_in_analysis: false })
        });
      }
      for (const item of currentEarly) await tableRows('intelligence_opportunities', 'on_conflict=owner_id,candidate_id,scope,package_name_zh', {
        method: 'POST', body: JSON.stringify({ owner_id: owner, candidate_id: candidate.id, hypothesis_id: item.hypothesis_id,
          project_id: null, scope: 'early', package_name_zh: item.opportunity_zh, participation_status: 'unverified',
          evidence_fact_number: item.evidence_fact_number, evidence_quote: item.evidence_quote,
          source_sha256: sourceHash, current_in_analysis: true, updated_at: new Date().toISOString() }),
        headers: { Prefer: 'resolution=merge-duplicates,return=minimal' }
      });
      for (const previous of previousProjects) await tableRows('intelligence_procurements',
        `owner_id=eq.${encodeURIComponent(owner)}&project_id=eq.${encodeURIComponent(previous.id)}&current_in_analysis=eq.true`,
        { method: 'PATCH', body: JSON.stringify({ current_in_analysis: false }) });
      return;
    }
    ...
    const projectRows = await tableRows('intelligence_projects', 'on_conflict=owner_id,candidate_id', {
      method: 'POST', body: JSON.stringify({ owner_id: owner, candidate_id: candidate.id,
        ...
        canonical_name: project.name_zh, stage_zh: project.stage_zh, current_in_analysis: true }),
      headers: { Prefer: 'resolution=merge-duplicates,return=representation' }
    });
    ...
    let packages = (extraction.commercial_events || []).filter(event => event.scope !== 'project').map(event => ({
      package_name_zh: event.object_zh, scope: event.scope, stage_code: event.stage, stage_zh: event.stage_text,
      evidence_fact_number: event.evidence_fact_number,
      evidence_quote: extraction.known_facts?.[event.evidence_fact_number - 1]?.evidence_quote || null,
      scope_text: event.scope_text, stage_text: event.stage_text, deadline_text: null
    }));
    const matchingPackages = packages.filter(item => item.package_name_zh === classification.procurement?.package_zh);
    if (matchingPackages.length === 1) matchingPackages[0].deadline_text = classification.procurement.deadline_text;
    if (classification.procurement?.package_zh && !matchingPackages.length) packages.push({
      package_name_zh: classification.procurement.package_zh, scope: 'unspecified', stage_code: null,
      stage_zh: classification.procurement.stage_zh, deadline_text: classification.procurement.deadline_text,
      evidence_fact_number: classification.procurement.evidence_fact_number,
      evidence_quote: extraction.known_facts?.[classification.procurement.evidence_fact_number - 1]?.evidence_quote || null,
      scope_text: null, stage_text: null
    });
    const grouped = new Map();
    for (const item of packages) {
      const key = JSON.stringify([item.scope, item.package_name_zh]);
      const group = grouped.get(key) || []; group.push(item); grouped.set(key, group);
    }
    packages = [...grouped.values()].map(group => new Set(group.map(item => item.stage_code)).size === 1 ? group[0] : {
      ...group[0], stage_code: null, stage_zh: '同一原文披露多个阶段，当前阶段未判定', evidence_fact_number: null,
      evidence_quote: [...new Set(group.map(item => item.evidence_quote).filter(Boolean))].join('\n'),
      stage_text: [...new Set(group.map(item => item.stage_text).filter(Boolean))].join(' / ')
    });
    // Only retire missing exact scope/name identities; never close sibling packages.
    const existingPackages = await tableRows('intelligence_procurements', procurementFilter + '&select=id,scope,package_name_zh,current_in_analysis');
    for (const previous of existingPackages) if (previous.current_in_analysis && !packages.some(item =>
      item.scope === previous.scope && item.package_name_zh === previous.package_name_zh)) {
      await tableRows('intelligence_procurements', procurementFilter + '&id=eq.' + encodeURIComponent(previous.id), {
        method: 'PATCH', body: JSON.stringify({ current_in_analysis: false })
      });
    }
    for (const item of packages) await tableRows('intelligence_procurements', 'on_conflict=owner_id,project_id,scope,package_name_zh', {
      method: 'POST', body: JSON.stringify({ owner_id: owner, project_id: savedProject.id, ...item, current_in_analysis: true }),
      headers: { Prefer: 'resolution=merge-duplicates,return=minimal' }
    });
    const opportunities = classification.disposition === 'candidate' ? packages.filter(item =>
      ['equipment', 'service'].includes(item.scope) && item.evidence_fact_number && item.evidence_quote) : [];
    const existingOpportunities = await tableRows('intelligence_opportunities', projectFilter + '&select=id,scope,package_name_zh,current_in_analysis');
    for (const previous of existingOpportunities) if (previous.current_in_analysis && !opportunities.some(item =>
      item.scope === previous.scope && item.package_name_zh === previous.package_name_zh)) {
      await tableRows('intelligence_opportunities', projectFilter + '&id=eq.' + encodeURIComponent(previous.id), {
        method: 'PATCH', body: JSON.stringify({ current_in_analysis: false })
      });
    }
    for (const item of opportunities) await tableRows('intelligence_opportunities', 'on_conflict=owner_id,candidate_id,scope,package_name_zh', {
      method: 'POST', body: JSON.stringify({ owner_id: owner, candidate_id: candidate.id, project_id: savedProject.id,
        scope: item.scope, package_name_zh: item.package_name_zh,
        participation_status: item.stage_code === 'open' ? 'public_tender_open'
          : ['awarded', 'signed'].includes(item.stage_code) ? 'package_awarded'
            : item.stage_code === 'cancelled' ? 'cancelled' : 'unverified',
        evidence_fact_number: item.evidence_fact_number, evidence_quote: item.evidence_quote,
        source_sha256: sourceHash, current_in_analysis: true, updated_at: new Date().toISOString() }),
      headers: { Prefer: 'resolution=merge-duplicates,return=minimal' }
    });
  ...
  return {
    ...
    async requestPasswordReset(email, redirectTo) {
      return request(`/auth/v1/recover?redirect_to=${encodeURIComponent(redirectTo)}`, {
        method: 'POST', publicKey: true, body: JSON.stringify({ email })
      });
    },
    async resetPassword(token, password) {
      return request('/auth/v1/user', { method: 'PUT', publicKey: true, token, body: JSON.stringify({ password }) });
    },
    ...
    async saveCandidate(sourceId, owner, extraction, sourceHash) {
      ...
      const tracked = await request('/rest/v1/rpc/sync_intelligence_tracking', { method: 'POST', body: JSON.stringify({
        p_owner_id: owner, p_candidate_id: candidate.id,
        p_hypotheses: classification.disposition === 'candidate' ? extraction.hypotheses : [],
        p_signals: classification.disposition === 'candidate' ? extraction.next_signals_zh : []
      }) });
      if (!tracked) throw failure('storage_failed');
      await materializeBusinessObjects(candidate, classification, owner, extraction, sourceHash);
      return candidate;
    },
    async candidates(owner) {
      ...
      const sources = candidates.length ? await rows(`owner_id=eq.${encodeURIComponent(owner)}&id=in.(${candidates.map(item => encodeURIComponent(item.source_id)).join(',')})&select=id,final_url,content_sha256,fetched_at,published_at,publication_date,publication_method,extraction_status,extraction_source_sha256,extraction_zh`) : [];
      ...
      const linked = await attachRelations(latest.map(item => {
        const source = sourceById.get(item.source_id);
        const extraction = source?.extraction_status === 'extracted' && source.extraction_source_sha256 === source.content_sha256
          ? source.extraction_zh : null;
        const sourceTiming = source ? { id: source.id, fetched_at: source.fetched_at, published_at: source.published_at,
          publication_date: source.publication_date, publication_method: source.publication_method } : null;
        return { ...item, source_timing: sourceTiming, why_it_matters_zh: extraction?.why_it_matters_zh || null,
          unknowns_zh: extraction?.unknowns_zh || [], next_signals_zh: extraction?.next_signals_zh || [],
          resilience_signal: extraction?.classification?.resilience_signal || null };
      }), owner);
      ...
      // Equal-scope identity is transitive; ordering and a late-arriving hub must not change counts.
      const byId = new Map(linked.map(item => [item.id, item]));
      const neighbors = new Map(linked.map(item => [item.id, new Set()]));
      for (const item of linked) for (const link of item.related_sources) {
        const peer = byId.get(link.related_candidate_id);
        if (!peer || item.disposition !== 'candidate' || peer.disposition !== 'candidate'
          || item.evidence_status === 'conflict' || peer.evidence_status === 'conflict'
          || item.review_status !== peer.review_status || link.same_scope !== true || link.relation !== 'supports') continue;
        neighbors.get(item.id).add(peer.id);
        neighbors.get(peer.id).add(item.id);
      }
      const grouped = [], visited = new Set();
      for (const item of linked) {
        if (visited.has(item.id)) continue;
        const ids = new Set([item.id]);
        for (const id of ids) for (const neighbor of neighbors.get(id)) ids.add(neighbor);
        ids.forEach(id => visited.add(id));
        const members = linked.filter(member => ids.has(member.id));
        const ambiguous = members.some(member => member.related_sources.some(link => ids.has(link.related_candidate_id)
          && (link.relation !== 'supports' || link.same_scope !== true)));
        if (ambiguous) {
          grouped.push(...members);
          continue;
        }
        const current = members.slice().sort((left, right) =>
          (Date.parse(right.source_timing?.publication_date) || 0) - (Date.parse(left.source_timing?.publication_date) || 0)
          || (Date.parse(right.source_timing?.fetched_at) || 0) - (Date.parse(left.source_timing?.fetched_at) || 0))[0];
        for (const field of ['radars', 'occurrence_countries', 'relevance_countries']) {
          current[field] = [...new Set(members.flatMap(member => member[field] || []))];
        }
        current.grouped_sources = members.map(member => ({ candidate_id: member.id, source_id: member.source_id }));
        grouped.push(current);
      }
      return grouped;
    },
    async currentOpportunities(owner, visibleCandidates) {
      const ids = [...new Set(visibleCandidates.filter(item => item.disposition === 'candidate' && item.review_status !== 'rejected')
        .flatMap(item => (item.grouped_sources || [{ candidate_id: item.id }]).map(source => source.candidate_id)))];
      if (!ids.length) return [];
      const idFilter = ids.map(encodeURIComponent).join(',');
      const [opportunities, candidates] = await Promise.all([
        tableRows('intelligence_opportunities', `owner_id=eq.${encodeURIComponent(owner)}&candidate_id=in.(${idFilter})&current_in_analysis=eq.true&select=id,candidate_id,hypothesis_id,scope,package_name_zh,participation_status,evidence_fact_number,evidence_quote,source_sha256,updated_at&order=updated_at.desc&limit=100`),
        tableRows('intelligence_candidates', `owner_id=eq.${encodeURIComponent(owner)}&id=in.(${idFilter})&select=id,source_id,title_zh,source_sha256,disposition,review_status,occurrence_countries`)
      ]);
      const earlyIds = [...new Set(opportunities.filter(item => item.scope === 'early' && item.hypothesis_id).map(item => item.hypothesis_id))];
      const liveHypotheses = earlyIds.length ? await tableRows('intelligence_hypotheses',
        `owner_id=eq.${encodeURIComponent(owner)}&id=in.(${earlyIds.map(encodeURIComponent).join(',')})&select=id,status`) : [];
      const activeEarly = new Set(liveHypotheses.filter(item => ['open', 'strengthened', 'weakened'].includes(item.status)).map(item => item.id));
      const byCandidate = new Map(candidates.map(item => [item.id, item]));
      const sourceIds = [...new Set(candidates.map(item => item.source_id))];
      const sources = sourceIds.length ? await rows(`owner_id=eq.${encodeURIComponent(owner)}&id=in.(${sourceIds.map(encodeURIComponent).join(',')})&select=id,content_sha256,extraction_source_sha256,extraction_status`) : [];
      const bySource = new Map(sources.map(item => [item.id, item]));
      const visible = opportunities.flatMap(item => {
        const candidate = byCandidate.get(item.candidate_id), source = bySource.get(candidate?.source_id);
        if (!candidate || candidate.disposition !== 'candidate' || candidate.review_status === 'rejected'
          || (item.scope === 'early' && !activeEarly.has(item.hypothesis_id))
          || !source || source.extraction_status !== 'extracted' || item.source_sha256 !== candidate.source_sha256
          || item.source_sha256 !== source.content_sha256 || source.extraction_source_sha256 !== source.content_sha256) return [];
        return [{ id: item.id, source_id: candidate.source_id, title_zh: candidate.title_zh,
          occurrence_countries: candidate.occurrence_countries, hypothesis_id: item.hypothesis_id, scope: item.scope, package_name_zh: item.package_name_zh,
          participation_status: item.participation_status, evidence_fact_number: item.evidence_fact_number,
          evidence_quote: item.evidence_quote, updated_at: item.updated_at }];
      });
      if (visible.length < 2) return visible;
      const links = await request('/rest/v1/rpc/current_intelligence_opportunity_links', {
        method: 'POST', body: JSON.stringify({ p_owner_id: owner })
      });
      const visibleById = new Map(visible.map(item => [item.id, item]));
      return visible.map(item => ({ ...item, related_sources: links.flatMap(link => {
        const peerId = link.left_opportunity_id === item.id ? link.right_opportunity_id
          : link.right_opportunity_id === item.id ? link.left_opportunity_id : null;
        const peer = visibleById.get(peerId);
        return peer ? [{ source_id: peer.source_id, participation_status: peer.participation_status }] : [];
      }) }));
    },
    ...
    async businessHistory(sourceId, owner) {
      await one(sourceId, owner);
      return tableRows('intelligence_business_history', `owner_id=eq.${encodeURIComponent(owner)}&source_id=eq.${encodeURIComponent(sourceId)}&select=id,project_id,procurement_id,source_sha256,snapshot,recorded_at&order=id.desc&limit=50`);
    },
    async notificationSettings(owner) {
      const result = await tableRows('intelligence_notification_settings', `owner_id=eq.${encodeURIComponent(owner)}&select=quiet_enabled,quiet_start_hour,quiet_end_hour,timezone,flash_breaks_quiet`);
      return result[0] || { quiet_enabled: true, quiet_start_hour: 23, quiet_end_hour: 7, timezone: 'Asia/Shanghai', flash_breaks_quiet: false };
    },
    async saveNotificationSettings(owner, settings) {
      const result = await tableRows('intelligence_notification_settings', 'on_conflict=owner_id', {
        method: 'POST', body: JSON.stringify({ ...settings, owner_id: owner }), headers: { Prefer: 'resolution=merge-duplicates,return=representation' }
      });
      const { owner_id, ...saved } = result[0];
      return saved;
    },
    ...
    async candidateBySource(sourceId, owner) {
      ...
      const [hypotheses, watches, opportunities, opportunityHistory, source] = await Promise.all([
        ...
        tableRows('intelligence_opportunities', `${scope}&select=id,hypothesis_id,scope,package_name_zh,participation_status,evidence_fact_number,evidence_quote,current_in_analysis,updated_at&order=updated_at.desc`),
        tableRows('intelligence_opportunity_history', `owner_id=eq.${encodeURIComponent(owner)}&source_id=eq.${encodeURIComponent(sourceId)}&select=id,opportunity_id,snapshot,recorded_at&order=id.desc&limit=50`),
        one(sourceId, owner)
      ]);
      ...
      return { ...candidate, opportunities, opportunity_history: opportunityHistory, tracking: { hypotheses: hypotheses.map(h => ({ ...h,
        current_in_analysis: require('./hypotheses.cjs').hypothesisInAnalysis(h, source.extraction_zh),
    ...
    async hypothesis(id, owner) {
      ...
      const source = await one(candidates[0].source_id, owner);
      return { ...result[0], candidate: candidates[0],
        current_in_analysis: require('./hypotheses.cjs').hypothesisInAnalysis(result[0], source.extraction_zh) };
    },
    ...
    async assessmentTargets(candidate, source, owner) {
      ...
      const hypotheses = await tableRows('intelligence_hypotheses', `owner_id=eq.${encodeURIComponent(owner)}&candidate_id=in.(${matched.map(p => encodeURIComponent(p.id)).join(',')})&status=in.(open,strengthened,weakened)&select=id,candidate_id,claim_zh,counter_evidence_zh&order=created_at.asc&limit=12`);
      return hypotheses.filter(h => require('./hypotheses.cjs').hypothesisInAnalysis(h,
        sources.find(s => s.id === matched.find(p => p.id === h.candidate_id)?.source_id)?.extraction_zh)).map(h => ({ id: h.id }));
    },
    ...
    async watchSearchTargets(owner, now = new Date()) {
      ...
      const [watches, sources] = await Promise.all([
        ...
        rows(`owner_id=eq.${encodeURIComponent(owner)}&id=in.(${candidates.map(c => encodeURIComponent(c.source_id)).join(',')})&status=eq.pending_extraction&select=id,final_url,title,extraction_zh`)
      ]);
      return hypotheses.flatMap(h => {
        ...
        return candidate && watch && source && require('./hypotheses.cjs').hypothesisInAnalysis(h, source.extraction_zh)
          ? [{ ...h, candidate, source_url: source.final_url, source_title: source.title }] : [];
      });
    },
    ...
    async enqueueDailyDigest(owner, jobId) {
      return request('/rest/v1/rpc/enqueue_intelligence_daily_digest', { method: 'POST', body: JSON.stringify({
        p_owner_id: owner, p_job_run_id: jobId
      }) });
    },
    ...
    async recordNotificationTarget(owner, id, payload, target) {
      if (!['chat', 'user'].includes(target?.target)) throw failure('upstream_unavailable');
      const delivery = { status: 'accepted', response_code: Number.isInteger(target.responseCode) ? target.responseCode : null };
      const nextPayload = { ...payload, _delivery_targets: { ...(payload?._delivery_targets || {}), [target.target]: delivery } };
      const result = await tableRows('intelligence_notification_outbox',
        `id=eq.${encodeURIComponent(id)}&owner_id=eq.${encodeURIComponent(owner)}&status=eq.sending&select=id,payload`, {
          method: 'PATCH', headers: { Prefer: 'return=representation' }, body: JSON.stringify({ payload: nextPayload })
        });
      if (!result?.[0]) throw failure('upstream_unavailable');
      Object.assign(payload, nextPayload);
      return result[0].payload;
    },
    async findCandidatePeers(candidate, owner) {
      const { plausibleSameProject, plausibleCorrectionPair, correctionContext, distinctSourcePublishers } = require('./crosscheck.cjs');
      ...
      for (const item of result) {
        const strictPeer = plausibleSameProject(candidate, item);
        if (!strictPeer && !correctionContext(candidate, item)) continue;
        const other = await one(item.source_id, owner, true);
        const differentPublisher = distinctSourcePublishers(source, other);
        if (!differentPublisher && !plausibleCorrectionPair({ ...candidate, extraction_zh: source.extraction_zh },
          { ...item, extraction_zh: other.extraction_zh })) continue;
        if (differentPublisher && !strictPeer) continue;
    ...
    async saveCrossCheck(candidateId, relatedCandidateId, owner, checked) {
      ...
      const projectIdentityId = relation === 'supports' && scope === true
        ? await request('/rest/v1/rpc/try_link_intelligence_project_identity', { method: 'POST', body: JSON.stringify({
          p_owner_id: owner, p_left_candidate_id: candidateId, p_right_candidate_id: relatedCandidateId
        }) }) : null;
      if (projectIdentityId) await request('/rest/v1/rpc/sync_intelligence_opportunity_links', {
        method: 'POST', body: JSON.stringify({
          p_owner_id: owner, p_left_candidate_id: candidateId, p_right_candidate_id: relatedCandidateId
        })
      });
      return { relation, project_identity_id: projectIdentityId };
    },
