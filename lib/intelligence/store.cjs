...
function createStore(config, fetchImpl = global.fetch) {
  ...
  return {
    ...
    async candidates(owner) {
      ...
      const sources = candidates.length ? await rows(`owner_id=eq.${encodeURIComponent(owner)}&id=in.(${candidates.map(item => encodeURIComponent(item.source_id)).join(',')})&select=id,final_url,fetched_at,published_at,publication_date,publication_method`) : [];
      ...
      const latest = candidates.filter(item => newest.get(sourceById.get(item.source_id)?.final_url || item.source_id) === item);
