...
function createStore(config, fetchImpl = global.fetch) {
  ...
  return {
    ...
    async candidates(owner) {
      ...
      const sources = candidates.length ? await rows(`owner_id=eq.${encodeURIComponent(owner)}&id=in.(${candidates.map(item => encodeURIComponent(item.source_id)).join(',')})&select=id,fetched_at,published_at,publication_date,publication_method`) : [];
      return attachRelations(candidates.map(item => ({ ...item, source_timing: sources.find(source => source.id === item.source_id) || null })), owner);
    },
