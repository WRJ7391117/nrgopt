...
function createStore(config, fetchImpl = global.fetch) {
  ...
  async function materializeBusinessObjects(candidate, classification, owner, extraction) {
  ...
  return {
    ...
    async saveCandidate(sourceId, owner, extraction, sourceHash) {
      ...
      await materializeBusinessObjects(candidate, classification, owner, extraction);
      return candidate;
    },
    ...
    async candidateBySource(sourceId, owner) {
      ...
      const [hypotheses, watches, source] = await Promise.all([
