...
function createStore(config, fetchImpl = global.fetch) {
  ...
  async function materializeBusinessObjects(candidate, classification, owner, extraction, sourceHash) {
    ...
    if (!project?.name_zh || !projectCountry) {
      ...
      await tableRows('intelligence_opportunities', projectFilter + '&current_in_analysis=eq.true', { method: 'PATCH', body: JSON.stringify({ current_in_analysis: false }) });
  ...
  return {
    ...
    async currentOpportunities(owner, visibleCandidates) {
      ...
      const [opportunities, candidates] = await Promise.all([
        tableRows('intelligence_opportunities', `owner_id=eq.${encodeURIComponent(owner)}&candidate_id=in.(${idFilter})&current_in_analysis=eq.true&select=id,candidate_id,scope,package_name_zh,participation_status,evidence_fact_number,evidence_quote,source_sha256,updated_at&order=updated_at.desc&limit=100`),
      ...
      return opportunities.flatMap(item => {
        ...
        return [{ id: item.id, source_id: candidate.source_id, title_zh: candidate.title_zh,
          occurrence_countries: candidate.occurrence_countries, scope: item.scope, package_name_zh: item.package_name_zh,
    ...
    async candidateBySource(sourceId, owner) {
      ...
      const [hypotheses, watches, opportunities, opportunityHistory, source] = await Promise.all([
        ...
        tableRows('intelligence_opportunities', `${scope}&select=id,scope,package_name_zh,participation_status,evidence_fact_number,evidence_quote,current_in_analysis,updated_at&order=updated_at.desc`),
