...
const PROVIDER_CONFIG_FIELDS = 'owner_id,capability,provider,endpoint,model,currency,reserve_micro,cross_check_reserve_micro,api_key_ciphertext,updated_at';
...
function settings(env = process.env) {
  ...
  return { url: url.origin, origin: origin.origin, anonKey: env.SUPABASE_ANON_KEY, serviceKey: env.SUPABASE_SERVICE_ROLE_KEY,
    adminId: env.NRGOPT_ADMIN_USER_ID, writes: writesRequested && productionAllowed };
}
...
function createStore(config, fetchImpl = global.fetch) {
  ...
  return {
    ...
    async operations(owner) {
      ...
      const budgets = await tableRows('intelligence_budget_accounts',
        `owner_id=eq.${encodeURIComponent(owner)}&select=currency,limit_micro,reserved_micro,spent_micro,period_start,period_end,enabled&order=currency.asc`);
