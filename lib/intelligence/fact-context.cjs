...
function validateNumericFacts(input, facts) {
  ...
  return input.map(item => {
    ...
    if (basis === 'it_load' && !/\bIT\b|information technology|信息技术|تقنية المعلومات|تكنولوجيا المعلومات/i.test(basisText)) throw fail(kind);
...
已明确具名项目且有设计/审批/认证文件等项目形成证据时，classification.radars应包含project；新增数据中心同时保留demand。设计文件认证不等于采购授标、施工完成或投运。known_facts每项只写其连续引文实际支持的事实，不把其他段落的地点、数量或口径塞进同一claim_zh；需要时拆开事实并分别引用。
it_load的basis_text必须明确含IT、information technology或对应原语言的“信息技术”口径；只有“AI数据中心100MW”不够。原文另有明确IT口径时必须引用那一句，而不是从其他事实借用口径。总IT负荷与每个机房的IT容量分别绑定各自引文和范围。
