...
function extractDocument(bytes, contentType, excerptLimit = 2_000) {
  ...
  if (mime === 'text/html') {
    ...
    // These article containers were verified against the fixed official-source corpus.
    // Prefer them to a whole-page main/body that can contain other projects' headlines.
    const article = $('.news-dt-body, .__news_details_sec, .news-listing.details .news-list, .news-page-holder.news-details-holder, .press-release-detail .content-sec').first();
...
async function fetchSource(input) {
  ...
  const collect = async () => {
    ...
    for (let redirects = 0; ; redirects += 1) {
      ...
      if (!document.excerpt) throw failure('source_empty_document', 422, 'Source contains no readable text.');
      return {
