...
// fsa.gov.om also omits its issuer; its Sectigo R46 root is in macOS trust but
// not the Node bundle. Both certificates are scoped to this publisher only.
const FSA_CA = [...rootCertificates,
...
function extractDocument(bytes, contentType, excerptLimit = 2_000, url = '') {
  ...
  if (mime === 'text/html') {
    ...
    const kappUrl = url ? new URL(url) : null;
    const kappArticle = kappUrl?.hostname === 'www.kapp.gov.kw' && /^\/media\/get_details\/\d+\/?$/.test(kappUrl.pathname)
      ? $('.blog-items > .blog-content > .item > .info.content-box').first() : null;
    const article = kappArticle?.length ? kappArticle : fsaArticle?.length ? fsaArticle
  ...
  return new Promise((resolve, reject) => {
    const request = https.get(url, {
      ...
      ca: ['omannews.gov.om', 'www.omannews.gov.om'].includes(url.hostname) ? OMAN_NEWS_CA
        : url.hostname === 'fsa.gov.om' ? FSA_CA
