...
function extractDocument(bytes, contentType, excerptLimit = 2_000) {
  ...
  if (mime === 'text/html') {
    ...
    const article = $('.news-dt-body, .__news_details_sec, .news-listing.details .news-list, .news-page-holder.news-details-holder, .press-release-detail .content-sec, .our-blog .entry-content, .com-content-article__body, .blogDetail-content').first();
...
function requestSource(url, address, signal) {
  return new Promise((resolve, reject) => {
    const request = https.get(url, {
      agent: false,
      ca: ['omannews.gov.om', 'www.omannews.gov.om'].includes(url.hostname) ? OMAN_NEWS_CA : undefined,
      signal,
...
async function fetchSource(input) {
  ...
  const collect = async () => {
    ...
    for (let redirects = 0; ; redirects += 1) {
      ...
      const document = extractDocument(response.bytes, response.contentType);
      if (!document.excerpt || isPlaceholderDocument(url.href, document.title)) {
