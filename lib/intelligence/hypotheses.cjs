...
function hypothesisInAnalysis(hypothesis, extraction) {
  return (extraction?.hypotheses || []).some(item => item.hypothesis_zh === hypothesis.claim_zh
    && (item.counter_evidence_zh || '') === (hypothesis.counter_evidence_zh || ''));
}
...
module.exports = { validateAssessment, createHypothesisEvaluator, hypothesisInAnalysis };
