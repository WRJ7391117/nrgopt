...
async function runPaidCall({ store, owner, jobId = null, operation, currency, budgetKey, providerMissingCode,
  profile = null, budgetLimitMicro = null, billingMode = 'balance', readBalance = null, env = process.env,
  ...
  const invoke = async reservationId => {
    // The immutable version refers to the exact profile captured before dispatch.
    if (!await store.startProviderCall(owner, reservationId, profile?.configVersionId)) {
      throw Object.assign(new Error('provider_call_not_started'), { code: 'provider_call_not_started', status: 409 });
    }
    callStarted = true;
    let result, error;
    try { result = await call(); } catch (caught) { error = caught; }
    const usage = result?.usage && Object.fromEntries(Object.entries(result.usage).filter(([key, value]) =>
      ['prompt_tokens', 'completion_tokens', 'prompt_cache_hit_tokens', 'prompt_cache_miss_tokens',
        'input_tokens', 'output_tokens', 'cache_creation_input_tokens', 'cache_read_input_tokens'].includes(key)
      && Number.isSafeInteger(value) && value >= 0));
    const errorCode = error ? (/^[a-z][a-z0-9_]{0,79}$/.test(error.code || '') ? error.code : 'provider_call_failed') : null;
    if (!await store.finishProviderCall(owner, reservationId, errorCode, usage || null)) {
      throw Object.assign(new Error('provider_call_record_pending'), { code: 'provider_call_record_pending', status: 502 });
    }
    if (error) throw error;
    return { ...result, configVersionId: profile?.configVersionId || null };
  };
  if (billingMode === 'included') {
    ...
    try { return await invoke(reservationId); }
    finally { if (callStarted) await store.releaseBudget(owner, reservationId); }
  }
  ...
  let result;
  let callError;
  try { result = await invoke(reservationId); }
  catch (error) { callError = error; }
  if (!callStarted) throw callError;
  let after;
