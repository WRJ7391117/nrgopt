...
async function runPaidCall({ store, owner, jobId = null, operation, currency, budgetKey, providerMissingCode,
  ...
  if (billingMode !== 'balance' || typeof readBalance !== 'function') {
    await store.releaseBudget(owner, reservationId);
  ...
  const chargedMicro = Math.max(before - after, 0);
  if (chargedMicro > amount) throw Object.assign(new Error('billing_sync_pending'), { code: 'billing_sync_pending', status: 502, cause: callError });
  if (chargedMicro > 0) await store.settleBudget(owner, reservationId, chargedMicro, 'actual');
  else await store.releaseBudget(owner, reservationId);
