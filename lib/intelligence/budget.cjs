...
function usageCharge(operation, result) {
  ...
  if (operation === 'discovery') {
    if (result.provider !== 'minimax' || (result.model || 'MiniMax-M3') !== 'MiniMax-M3') return null;
    ...
    return {
      ...
      provider: result.provider, model: result.model || 'MiniMax-M3',
  ...
  if (operation === 'extraction' || operation === 'cross_check') {
    if (result.provider !== 'deepseek' || (result.model || 'deepseek-flash') !== 'deepseek-flash') return null;
    ...
    return {
      ...
      provider: result.provider, model: result.model || 'deepseek-flash',
...
async function runPaidCall({ store, owner, jobId = null, operation, currency, budgetKey, providerMissingCode,
  reserveMicro = null, env = process.env, idempotencyKey = `manual:${operation}:${randomUUID()}`, call }) {
  if (!['CNY', 'USD'].includes(currency)) throw Object.assign(new Error('budget_not_configured'), { code: 'budget_not_configured', status: 503 });
  const amount = reserveMicro == null ? reserveAmount(env, budgetKey) : reserveAmount({ value: reserveMicro }, 'value');
